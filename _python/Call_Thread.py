...
def preview(self):
    ...
    if General.image_format:
        self.Preview_Thread.finished.connect(
            lambda: UI_Update.update_preview_frame(
                self, "../_temp/preview.jpg")
        )
        self.Preview_Thread.finished.connect(
        ...
        self.Preview_Thread.finished.connect(
            lambda: UI_Update.update_preview_frame(
                self, "../_temp/preview.png")
        )
        self.Preview_Thread.finished.connect(
            lambda: os.system("gpicview ../_temp/preview.png")
        )
    self.Preview_Thread.finished.connect(
        lambda: UI_Update.error_UI_update(self))
...
def timelapse(self):
    ...
    if not General.timelapse_thread_running:
        ...
        self.Timelapse_Thread.complete.connect(
            lambda: UI_Update.update_preview_frame(
                self, General.current_full_file_name)
        )
...
def ambient_sensors(self):
    if not General.ambient_thread_running:
        ...
        self.Ambient_Thread.started.connect(
            lambda: UI_Update.ambient_UI_update(self))
        self.Ambient_Thread.started.connect(
...
def soil_sensors(self):
    if not General.soil_thread_running:
        ...
        self.Soil_Thread.started.connect(
            lambda: UI_Update.soil_UI_update(self))
        self.Soil_Thread.started.connect(
            lambda: UI_Update.soil_sensor_reset(self))
        self.Soil_Thread.finished.connect(
            lambda: UI_Update.soil_UI_update(self))
        self.Soil_Thread.error.connect(lambda: Sensors.soil_sensor_reset(self))
